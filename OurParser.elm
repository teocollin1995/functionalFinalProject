...
-- use the second parser only when the first fails
left_or : Parser result -> Parser result -> Parser result
left_or p q =
  Direct <| \xs ->
    case funP p xs of
      [] -> funP q xs
...
chainl p op a = chainl1 p op `or` succeed a

chainr : Parser a -> Parser (a -> a -> a) -> a -> Parser a
chainr p op a  = chainr1 p op `or` succeed a
